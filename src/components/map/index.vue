...
export default {
  ...
  methods:{
    ...
    circle(obj){
      ...
      if(obj.text){
        if(obj.text.show)
          this.map[obj.name].bindPopup(obj.text.content).openPopup();
        else
          this.map[obj.name].bindPopup(obj.text.content);
      }
    },
    //  经纬度上图
    marker(obj){
  ...
  mounted() {
    ...
    map=L.map('map',{
      ...
      // center: [22.87, 113.55],
      // zoom: 10
    }).setView([19.38888634723282, 110.12145996093751],_this.info.zoom);
    _this.scaleFn(_this.info.zoom);
    ...
    // this.heatmap();
    map.on("zoomend", e => {
      _this.info.zoom=e.target.getZoom();
      _this.scaleFn(_this.info.zoom);
    })
